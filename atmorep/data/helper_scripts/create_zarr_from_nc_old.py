...
nc_index = { 'vorticity' : 'vo', 'divergence' : 'd', 'geopotential' : 'z',
...
#fields = ['velocity_u', 'velocity_v', 'temperature']
...
path_base = '/work/ab1385/a270082/era_corrected'
fname_base = path_base + '/T2M_2021_xr1.nc'
...
def get_data(field, fname, range, upsampling_factor):
  print(fname)
  ds_nc = xr.open_dataset(fname, 
                          engine='netcdf4')

  fn = nc_index[ field ]
  data = ds_nc[ fn ].values[ range[0]:range[1] ]
  data = reduce_resolution(data, upsampling_factor)

  valid_time = ds_nc['valid_time'].values.astype('datetime64[s]')
...
if __name__ == "__main__":
  ...
  if year_start != year_end:
    ...
    fname = '/work/ab1412/atmorep/data/era_corrected/era5_y{}_{}_res{:03d}_chunk8.zarr'.format(year_start, year_end, int(res[0]*100))
  else:
    #fname = '/p/scratch/atmo-rep/data/era5_1deg/months/era5_y{}_res{:03d}_chunk8.zarr'.format(year_start, int(res[0]*100))
    fname = '/work/ab1412/atmorep/data/era_corrected/era5_y{}_res{:03d}_chunk8.zarr'.format(year_start, int(res[0]*100))
    ...
    for month in range( 1, 12+1) :
      ...
      for ilvl, level in enumerate(levels) :
        for ifld, field in enumerate(fields) :
          ...
          ds_grib = xr.open_dataset( fname_base.format( field, level, field, year, month, level), 
                                     engine='cfgrib',
      ...
      for ifld_sfc, field_sfc in enumerate(fields_sfc) :
        ...
        ds_grib = xr.open_dataset( fname_base.format(field_sfc, 0, field_sfc, year, month, 0), 
                                      engine='cfgrib',
                                      backend_kwargs={'time_dims':('valid_time','indexing_time'),  "indexpath": ''} )

        fn = nc_index[ field_sfc ]  
