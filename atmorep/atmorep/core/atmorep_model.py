...
class AtmoRepData( torch.nn.Module) :
  ...
  ###################################################
  def set_global( self, mode : NetMode, times, batch_size = -1, num_loader_workers = -1, lat_range = None) :

    cf = self.net.cf
    if batch_size < 0 :
      batch_size = cf.batch_size_train if mode == NetMode.train else cf.batch_size_test
    dataset = self.dataset_train if mode == NetMode.train else self.dataset_test
    dataset.set_global( times, batch_size, cf.token_overlap, lat_range = lat_range)
